...
LOG_DIR = "2d/pygame/PPOVecFrame/logs_opt/"
OPT_DIR = "2d/pygame/PPOVecFrame/opt/"
...
def train_model(
    ...
    print(
        "Tensorboard command: python3 -m tensorboard.main --logdir=2d/pygame/PPOVecFrame/tensorboard/{}/".format(
            model_name
        )
    )
    ...
    model = PPO(
        ...
        batch_size=512,
        tensorboard_log="2d/pygame/PPOVecFrame/tensorboard/{}".format(model_name),
        gamma=gamma,
        gae_lambda=gae_lambda,
    )

    model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False)
    model.save("2d/pygame/PPOVecFrame/trained/{}".format(model_name))
...
def train_existing_model(
    ...
    env = make_vec_env(partial(EnvRL, dim, False), n_envs=n_envs)
    model = PPO.load(
        "2d/pygame/PPOVecFrame/trained/{}".format(model_name),
        env=env,
        device="cpu",
        verbose=verbose,
        learning_rate=learning_rate,
    ...
    for i in range(n_iters):
        model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False)
        model.save(
            "2d/pygame/PPOVecFrame/trained/{}{}".format(
...
def test_model(model_name, render=False, verbose=True):
    ...
    model = PPO.load("2d/pygame/PPOVecFrame/trained/{}".format(model_name))
...
if __name__ == "__main__":
    ...
    train_model(2, 32, 250000, "test", gamma=0.991)
