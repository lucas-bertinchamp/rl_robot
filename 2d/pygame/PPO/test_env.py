...
def train_model(dim, n_envs, n_timesteps, model_name):
    ...
    model.save("2d/pygame/PPO/trained/{}".format(model_name))


def train_existing_model(dim, n_envs, n_timesteps, model_name, n_iters):
    env = make_vec_env(partial(EnvRL, dim), n_envs=n_envs)
    model = PPO.load(
        "2d/pygame/PPO/trained/{}".format(model_name),
        env=env,
        device="cpu",
        verbose=1,
    )
    ...
    for i in range(n_iters):
        model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False)
        model.save("2d/pygame/PPO/trained/{}{}".format(model_name, (i + 1) * n_timesteps))


def test_model(model_name, render=False, verbose=True):
    env = EnvRL(2)
    model = PPO.load("2d/pygame/PPO/trained/{}".format(model_name))
    obs = env.reset()
...
if __name__ == "__main__":
    ...
    test_model("targetFixe", render=True, verbose=True)
