...
def train_model(
    ...
    print(
        "Tensorboard command: python3 -m tensorboard.main --logdir=2d/pygame/PPOv3/tensorboard/{}/".format(
            model_name
        )
    )
    ...
    model = PPO(
        ...
        tensorboard_log="2d/pygame/PPOv3/tensorboard/{}".format(model_name),
        gamma=gamma,
    )

    model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False)
    model.save("2d/pygame/PPOv3/trained/{}".format(model_name))
...
def train_existing_model(
    ...
    env = make_vec_env(partial(EnvRL, dim), n_envs=n_envs)
    model = PPO.load(
        "2d/pygame/PPOv3/trained/{}".format(model_name),
        env=env,
        device="cpu",
        verbose=verbose,
        learning_rate=learning_rate,
    ...
    for i in range(n_iters):
        model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False)
        model.save(
            "2d/pygame/PPOv3/trained/{}{}".format(model_name, (i + 1) * n_timesteps)
        )


def test_model(model_name, render=False, verbose=True):
    env = EnvRL(2)
    model = PPO.load("2d/pygame/PPOv3/trained/{}".format(model_name))
    obs = env.reset()
