...
from envRLsac import EnvRL

import gym
import numpy as np
from stable_baselines3.common.vec_env import DummyVecEnv, VecFrameStack, SubprocVecEnv
from stable_baselines3 import SAC, DQN, SAC
...
LOG_DIR = "2d/pygame/SAC/logs_opt/"
OPT_DIR = "2d/pygame/SAC/opt/"
...
def train_model(dim, n_envs, n_timesteps, model_name, learning_rate=0.0003, gamma=0.99):
    print(
        "Tensorboard command: python3 -m tensorboard.main --logdir=2d/pygame/SAC/tensorboard/{}/".format(
            model_name
        )
    )
    ...
    model = SAC(
        ...
        env,
        device="cpu",
        tensorboard_log="2d/pygame/SAC/tensorboard/{}".format(model_name),
        verbose=1,
        batch_size=1024,
    )

    model.learn(total_timesteps=n_timesteps, reset_num_timesteps=False, log_interval=5)
    model.save("2d/pygame/SAC/trained/{}".format(model_name))
...
def train_existing_model(
    ...
    env = make_vec_env(partial(EnvRL, dim, False), n_envs=n_envs)
    model = SAC.load(
        "2d/pygame/SAC/trained/{}".format(model_name),
        env=env,
        device="cpu",
        verbose=verbose,
    ...
    for i in range(n_iters):
        ...
        model.save(
            "2d/pygame/SAC/trained/{}{}".format(model_name, (i + 1) * n_timesteps)
        )
...
def test_model(model_name, render=False, verbose=True):
    ...
    model = SAC.load("2d/pygame/SAC/trained/{}".format(model_name))
...
if __name__ == "__main__":
    ...
    test_model("SACobstacles2", render=True, verbose=True)
